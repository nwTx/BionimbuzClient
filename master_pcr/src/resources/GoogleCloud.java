...
import resources.StorageInstance;
...
public class GoogleCloud {
	...
	final String defaultConfigPathnameInstance = "D:\\Dropbox\\Mestrado\\spcr\\data\\googleCloudInstance.json";
	final String defaultConfigPathnameStorage = "D:\\Dropbox\\Mestrado\\spcr\\data\\googleCloudStorage.json";
	...
	private String allInstanceTypeName[] = { "F1.MICRO", "G1.SMALL", "N1.STANDARD-1", "N1.STANDARD-2", "N1.STANDARD-4",
			...
			"N1.HIGHCPU-32", "F1.MICRO.PREEMPTIBLE", "G1.SMALL.PREEMPTIBLE", "N1.STANDARD-1.PREEMPTIBLE",
			"N1.STANDARD-2.PREEMPTIBLE", "N1.STANDARD-4.PREEMPTIBLE", "N1.STANDARD-8.PREEMPTIBLE",
			"N1.STANDARD-16.PREEMPTIBLE", "N1.STANDARD-32.PREEMPTIBLE", "N1.HIGHMEM-2.PREEMPTIBLE",
			"N1.HIGHMEM-4.PREEMPTIBLE", "N1.HIGHMEM-8.PREEMPTIBLE", "N1.HIGHMEM-16.PREEMPTIBLE",
			"N1.HIGHMEM-32.PREEMPTIBLE", "N1.HIGHCPU-2.PREEMPTIBLE", "N1.HIGHCPU-4.PREEMPTIBLE",
			"N1.HIGHCPU-8.PREEMPTIBLE", "N1.HIGHCPU-16.PREEMPTIBLE", "N1.HIGHCPU-32.PREEMPTIBLE" };
	private String allLocation[] = { "asia", "europe", "us" };
	// private String allStorage[] = {CPCOMPUTEENGINESTORAGEPDCAPACITY,
	// CPCOMPUTEENGINESTORAGEPDSSD, CPCOMPUTEENGINEPDIOREQUEST,
	// CPCOMPUTEENGINESTORAGEPDSNAPSHOT};
	 ...
	 *
	 * @param server
	 *            Server where the JSON file is located.
	 * @param index
	 *            Remaining URL info.
	 * @throws IOException
	 */
	...
	public GoogleCloud() {
		...
		try (InputStream is = new FileInputStream(this.defaultConfigPathnameInstance);) {
			...
			// Logger.getLogger(GoogleCloud.class.getName()).log(Level.SEVERE,
			// null, ex);
		}
	...
	private JSONObject getGcpInstance() {
		// this.computeEngine =
		// JsonReader.readJson(defaultConfigPathnameInstance);
	...
	private JSONObject getGcpStorage() {
		...
		try {
			result.put(CPCOMPUTEENGINESTORAGEPDCAPACITY,
					this.storageEngine.getJSONObject(CPCOMPUTEENGINESTORAGEPDCAPACITY));
			result.put(CPCOMPUTEENGINEPDIOREQUEST, this.storageEngine.getJSONObject(CPCOMPUTEENGINEPDIOREQUEST));
			result.put(CPCOMPUTEENGINESTORAGEPDSNAPSHOT,
					this.storageEngine.getJSONObject(CPCOMPUTEENGINESTORAGEPDSNAPSHOT));
			result.put(CPCOMPUTEENGINESTORAGEPDSSD, this.storageEngine.getJSONObject(CPCOMPUTEENGINESTORAGEPDSSD));
		} catch (NullPointerException | JSONException ex) {
			result.put(CPCOMPUTEENGINESTORAGEPDCAPACITY,
					this.computeEngine.getJSONObject(GCP_PRICE_LIST).getJSONObject(CPCOMPUTEENGINESTORAGEPDCAPACITY));
			result.put(CPCOMPUTEENGINEPDIOREQUEST,
					this.computeEngine.getJSONObject(GCP_PRICE_LIST).getJSONObject(CPCOMPUTEENGINEPDIOREQUEST));
			result.put(CPCOMPUTEENGINESTORAGEPDSNAPSHOT,
					this.computeEngine.getJSONObject(GCP_PRICE_LIST).getJSONObject(CPCOMPUTEENGINESTORAGEPDSNAPSHOT));
			result.put(CPCOMPUTEENGINESTORAGEPDSSD,
					this.computeEngine.getJSONObject(GCP_PRICE_LIST).getJSONObject(CPCOMPUTEENGINESTORAGEPDSSD));
		}
		return (result);
	}
	...
	// try {
	// System.out.print("maxNumberOfPd: " + i.getInt("maxNumberOfPd") +
	// "maxPdSize: " + i.getInt("maxPdSize") + ", ");
	...
	// System.out.println("Price asia-northeast: " +
	// i.getDouble("asia-northeast") + " ,");
